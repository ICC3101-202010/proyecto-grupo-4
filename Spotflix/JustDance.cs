...
namespace Spotflix
{
    class JustDance 
    {
        ...
        public JustDance(int nofVideos, List<Video> videos)
        {
        ...
        public void Add(Video video) //No se si esto tiene que ir acá porque el administrador lo debería manejar nomas
        {
            ...
            {
                videos.Add(video);
                Console.WriteLine($"Se ha agregado el video {video.Name} a Just Dance");
            }
            else
            {
                Console.WriteLine("El video ya es parte de Just Dance. ¿Desea agregarlo de todas formas?\nOpción 1: Sí\nOpción 2: No");
        ...
        public void Delete(Video video) //No se si esto tiene que ir acá porque el administrador lo debería manejar nomas
        {
            foreach (Video v in videos)
            {
                ...
                {
                    videos.Remove(video);
                    Console.WriteLine($"Se ha eliminado el video {video.Name} de Just Dance");
                }
            }
        }
    }
